...
export class IndividualComponent extends EventEmitter implements OnInit {
  customEvent:boolean;
...
photoFound: boolean | undefined;
  ...
  contact2address_gid: any;
    ...
    console.log(this.application_gid);
...
this.Form_60Array = loanManagementModel.Form_60Array
   ...
   this.router.navigate( [ 'app/application-creation/Borrower Details' ], { fragment: this.subMenuTitle } );
    ...
    // this.handleFragmentChange(null);
    this.loadForm();
...
get familyAge(){
  ...
  selectAddressTypes(addresstype:any){
    ...
    this.addressTypesDropdown = addresstype.address_gid;
  ...
  addAddressDetails(){
    debugger
    var urladdressadd= "MstApplicationAdd/AddressAdd";
    ...
    switch (this.addressDetailsFormGroup.invalid) {
      ...
      var geocodeurl = 'GoogleMapsAPI/GetGeoCoding';
      this.socketService.getparams(geocodeurl,addressparams).subscribe((responsegeocode: any) => {
        ...
        this.params = {
          ...
          addresstype_name:this.addressDetailsFormGroup.value.addressType,
          addresstype_gid:this.addressTypesDropdown,
...
delAddData(contact2address_gid:any){
  debugger
  var urldeleteaddress = "MstApplicationAdd/AddressDelete";
...
addresslist(){
  var urlAddressaddedlist = "MstNgApplicationAdd/GetAddressList";  

debugger
        this.socketService.get(urlAddressaddedlist).subscribe((responseAddressaddedlist: any) => {
          debugger
          this.addressTable = [];
          for (var i = 0; i < responseAddressaddedlist.contactaddress_list.length; i++) {
            if (responseAddressaddedlist.contactaddress_list[i] != null) {
                //photoUrlArray[i] = resp.data[i];
           
          var addressArray  = [
            responseAddressaddedlist.contactaddress_list[i].addressline1,
            responseAddressaddedlist.contactaddress_list[i].addressline2,
            responseAddressaddedlist.contactaddress_list[i].taluka,
            responseAddressaddedlist.contactaddress_list[i].district,
            '-'+responseAddressaddedlist.contactaddress_list[i].postal_code
          ]
      debugger
          const addressDetailsObject = {
            contact2address_gid: responseAddressaddedlist.contactaddress_list[i].contact2address_gid,
            address_type: responseAddressaddedlist.contactaddress_list[i].addresstype_name,
            address: addressArray,
            landmark: responseAddressaddedlist.contactaddress_list[i].landmark,
            primaryStatus:responseAddressaddedlist.contactaddress_list[i].primary_status,
            longitude:responseAddressaddedlist.contactaddress_list[i].longitude,
            latitude:responseAddressaddedlist.contactaddress_list[i].latitude,
            addressLine1:responseAddressaddedlist.contactaddress_list[i].addressline1,
            addressLine2:responseAddressaddedlist.contactaddress_list[i].addressline2,
            postalCode:responseAddressaddedlist.contactaddress_list[i].postal_code,
          }
          this.addressTable.push(addressDetailsObject);
        }
      }
  ...
  //     this.application_gid; // Provide the actual value for application_gid 
  //     const encryptedParameter = this.cmnfunctionService.encryptURL('lsapplication_gid=' + this.application_gid + '&lspage='+cardFragment);
  //     const url = 'app/application-creation/Borrower Details?hash=' + encryptedParameter;
  //   this.router.navigateByUrl(url).then(() => {
  //     this.handleFragmentChange(cardFragment);
  //   });
  // }
  parallaxCardClick(cardFragment:any){
  ...
  handleFragmentChange(fragment:any)
  {
    if(fragment !== null){
      this.subMenuTitle = fragment;
      this.basicFormGroup;
  ...
  nextButton(){
    if(this.subMenuTitle == 'Basic'){
      ...
      var params={
        lsapplication_gid: this.application_gid ,
      ...
      this.applicationcreation.post(url, params).subscribe((result:any)=>{
        ...
        if (result.status == true) {
          ...
          this.application_gid=result.application_gid;
            console.log(this.application_gid);
            this.addresslist();
          this.application_gid
        }
        else {
...
if(this.subMenuTitle == 'Address'){
      switch (this.addressTable.length == 0) {
        case true:
          this.notify.showToastMessage('success',"Please add Address");
          break;
          this.addresslist()
          this.params = {
            application_gid:this.contact2address_gid,
            
        }
        var addresssubmiturl = 'MstNgApplicationAdd/SubmitIndividualAddressDtl';
        this.application.uilock();
      
        this.socketService.post(addresssubmiturl,this.params).subscribe((result:any) => {
          this.application.uiunlock();
          if(result.status == true){
            this.notify.showToastMessage('success',result.message);
  ...
  this.filternearestSamunnatiBranchTypes = this.nearestSamunnatiBranches.filter(
    ...
    .subscribe((value: string) => {
      this.filteraddressTypes = this.addressTypes.filter(
        (Adress: { addresstype_name: string }) =>
          Adress.addresstype_name.toLowerCase().includes(value.toLowerCase())
      );
    });
