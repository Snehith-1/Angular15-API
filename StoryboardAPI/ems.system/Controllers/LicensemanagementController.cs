...
{
    [RoutePrefix("api/Licensemanagement")]
    [Authorize]

    public class LicensemanagementController : ApiController
    {

        session_values Objgetgid = new session_values();
        logintoken getsessionvalues = new logintoken();
        DaLicensemanagement objDaCustomer = new DaLicensemanagement();

        // Customer Add
        [ActionName("PostLicensemanagement")]
        [HttpPost]
        public HttpResponseMessage PostLicensemanagement(licensemanagement_list values)
        {
            string token = Request.Headers.GetValues("Authorization").FirstOrDefault();
            getsessionvalues = Objgetgid.gettokenvalues(token);
            objDaCustomer.DaPostLicensemanagement(getsessionvalues.user_gid, values);
